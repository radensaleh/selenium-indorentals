...
const INIT_URL = "https://stag.indo.rentals/login"
const REGIS_URL = "https://stag.indo.rentals/register"
const TIME_1000 = 1000

async function Registration(driver, owner) {
    // open url
    await driver.get(INIT_URL)

    // open url registration
    await driver.get(REGIS_URL)

    // open url partner register
    if (!owner) {
        await driver.get("https://stag.indo.rentals/partner/register")
    } else {
        // open url owner register
        await driver.get("https://stag.indo.rentals/owner/register")
    }
    ...
    if (!owner) {
        ...
        // submit KTP
        var base64_txt = fs.readFileSync("./registration/base64_ktp.txt").toString('utf-8')
        var base64 = await driver.findElement(By.name("base64"))
        driver.executeScript(`arguments[0].value='${base64_txt}';`, base64)
    } else {
        // select file upload
        await driver.findElement(By.id('ktp')).sendKeys('/Users/radensaleh/Documents/Image Testing/ktp.jpg')
    }
    ...
    if (owner) { await driver.findElement(By.xpath("//input[@id='businessName']")).sendKeys("Mandiri Rental Jaya") }
    await driver.findElement(By.xpath("//input[@id='phone']")).sendKeys("089777217122")
    ...
    if (owner) {
        await driver.findElement(By.xpath("//input[@id='email']")).sendKeys(`selenium.owner${randInt}@gmail.com`)
    } else {
        await driver.findElement(By.xpath("//input[@id='email']")).sendKeys(`selenium.partner${randInt}@gmail.com`)
    }
    await driver.findElement(By.xpath("//input[@id='password']")).sendKeys("password")
    await driver.findElement(By.xpath("//input[@id='passwordConfirmation']")).sendKeys("password")
    await driver.findElement(By.id("buttonSubmit")).click()

    await driver.sleep(3 * TIME_1000)
    await driver.get("https://stag.indo.rentals/login")
    if (owner) { console.log('Registration Owner Success!') } else { console.log('Registration Partner Success!') }
    // driver.quit()
}
